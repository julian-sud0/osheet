...
export interface Item {
  ...
  input: { type: 'slider'; min: 0; max: 100 } | { type: 'likert'; min: 1; max: 7; labels: [string, string] };
}
...
const IBS_SSS_ITEMS: Item[] = [
  {
    ...
    reveal: 'Scored on a 0–100 scale, the standard used by gastroenterologists.',
    input: { type: 'slider', min: 0, max: 100 },
  },
  {
    prompt: 'On how many of the last 10 days did you have abdominal pain?',
    why: 'Frequency and severity are scored separately because they tell different stories.',
    reveal: 'On the IBS-SSS, this contributes 0–100 points (each day = 10 points).',
    input: { type: 'slider', min: 0, max: 100 },
  },
  {
    ...
    reveal: 'Adds another 0–100 to the total. Pain + bloating commonly co-vary.',
    input: { type: 'slider', min: 0, max: 100 },
  },
  {
    prompt: 'How satisfied have you been with your bowel habits?',
    why: 'Bowel habit dissatisfaction reflects the lived experience of unpredictability.',
    reveal: 'Inverted scoring — higher score = less satisfied. Adds 0–100.',
    input: { type: 'slider', min: 0, max: 100 },
  },
  {
